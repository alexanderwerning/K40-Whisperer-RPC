...
class G_Code_Rip:
    ...
    def Read_G_Code(self, filename, XYarc2line=False, arc_angle=2, units="in", Accuracy=""):
        ...
        mvtype = 1
        # G17 (Z-axis, XY-plane), G18 (Y-axis, XZ-plane), or G19 (X-axis, YZ-plane)
        plane = "17"
        pos = ['', '', '']
        pos_last = ['', '', '']
        ...
        for line in fin:
            ...
            if line.find("[") != -1 and line.find("[") != 0:
                ...
                while s != -1:
                    ...
                    while val > 0:
                        if e >= len(line):
                            MSG = "ERROR: Unable to evaluate expression: G-Code Line %d" % (
                                line_number)
            ...
            for code in UCODES:
                if line.find(code) != -1:
                    READ_MSG.append(
                        "Warning: %s Codes are not supported ( G-Code File Line: %d )" % (code, line_number))
                    skip = True
            if skip:
                continue
            ...
            for com in code_line:
                if com[0] == "G":
                    ...
                    if Gnum == "0" or Gnum == "1":
                        ...
                        READ_MSG.append(
                            "Warning: G%s Codes are not supported ( G-Code File Line: %d )" % (Gnum, line_number))
                    elif Gnum == "90.1":
                        mode_arc = "absolute"
                ...
                ###################
                    ...
                    READ_MSG.append(
                        "Aborting G-Code Reading: P word specifying the number of full or partial turns of arc are not supported")
                    return READ_MSG
            ...
            if mv_flag == 1:
                ...
                if mvtype == 1:
                    self.g_code_data.append(
                        [mvtype, pos_last[:], pos[:], feed, spindle])
                if mvtype == 2 or mvtype == 3:
                    if plane == "17":
                        if XYarc2line == False:
                            self.g_code_data.append(
                                [mvtype, pos_last[:], pos[:], center[:], feed, spindle])
                        else:
                            data = self.arc2lines(
                                pos_last[:], pos[:], center[:], mvtype, plane)

                            for line in data:
                                XY = line
                                self.g_code_data.append(
                                    [1, XY[:3], XY[3:], feed, spindle])
        ...
        while ((cnt < len(self.g_code_data)) and (isinstance(first_sum, complex))):
            ...
            if line[0] == 0 or line[0] == 1 or line[0] == 2 or line[0] == 3:
                if (isinstance(firstx, complex)):
                    firstx = line[2][0]
                if (isinstance(firsty, complex)):
                    firsty = line[2][1]
                if (isinstance(firstz, complex)):
                    firstz = line[2][2]
            cnt = cnt+1
            first_sum = firstx + firsty + firstz
        max_cnt = cnt
    ...
    def get_center(self, POS, POS_LAST, Rin, mvtype, plane="17"):
        ...
        CENTER = ["", "", ""]
        cord = sqrt((POS[xind]-POS_LAST[xind])**2 +
                    (POS[yind]-POS_LAST[yind])**2)
        v1 = cord/2.0
        ...
        theta = self.Get_Angle2(
            POS[xind]-POS_LAST[xind], POS[yind]-POS_LAST[yind])
        ...
        for line in code2split:
            ...
            if mvtype >= 1 and mvtype <= 3:
                ...
                if mvtype == 2 or mvtype == 3:
                    ...
                    cross = self.get_arc_intersects(
                        pos_last[:], pos[:], xsplit, center[:], "G%d" % (mvtype))
    ...
    #######################################
    # ,Xoffset,Yoffset):
    def probe_code(self, code2probe, nX, nY, probe_istep, minx, miny, xPartitionLength, yPartitionLength):
                ...
                if mvtype == 1:
                    ...
                    if (length <= min_length):
                        ...
                        for n in range(1, Lsteps+1):
                            ...
                            out.append([mvtype, [xstp0, ystp0, zstp0], [
                                       xstp1, ystp1, zstp1], feed, spindle])
                            xstp0 = float(xstp1)
                            ystp0 = float(ystp1)
                            zstp0 = float(zstp1)
        ...
        for i in range(len(out)):
            ...
            if mvtype >= 1 and mvtype <= 3:
                ...
                i_x, i_y = self.get_ix_iy(
                    (pos_last[0]-minx), (pos_last[1]-miny), xPartitionLength, yPartitionLength)
                #i_x = i_x+Xoffset
                #i_y = i_y+Yoffset
                if i_x < 0:
                    i_x = 0
                ...
                Xfraction = ((pos_last[0]-minx) -
                             (i_x*xPartitionLength))/xPartitionLength
                Yfraction = ((pos_last[1]-miny) -
                             (i_y*yPartitionLength))/yPartitionLength
                ...
                #### ADD ADDITIONAL DATA TO POS_LAST DATA ####
                i_x, i_y = self.get_ix_iy(
                    (pos[0]-minx), (pos[1]-miny), xPartitionLength, yPartitionLength)
                #i_x = i_x+Xoffset
                #i_y = i_y+Yoffset
                if i_x < 0:
                    i_x = 0
                ...
                p_index_D = int(i_y2*nX + i_x2)
                Xfraction = (
                    (pos[0]-minx)-(i_x*xPartitionLength))/xPartitionLength
                Yfraction = (
                    (pos[1]-miny)-(i_y*yPartitionLength))/yPartitionLength
        ...
        for line in code2scale:
            ...
            if mvtype >= 0 and mvtype <= 3:
                ...
                if mvtype == 2 or mvtype == 3:
                    ...
                    if mvtype == 3:
                        ang1 = self.Get_Angle2(
                            pos_last[0]-center[0], pos_last[1]-center[1])
                        xtmp, ytmp = self.Transform(
                            pos[0]-center[0], pos[1]-center[1], radians(-ang1))
                        ...
                        xtmp, ytmp = self.Transform(
                            pos_last[0]-center[0], pos_last[1]-center[1], radians(-ang1))
                        ang2 = self.Get_Angle2(xtmp, ytmp)

                    if ang2 == 0:
                        ang2 = 359.999

                    Radius = sqrt((pos[0]-center[0])**2 +
                                  (pos[1]-center[1])**2)
    ...
    def generategcode(self, side, z_safe=.5,
        ...
        if Wrap == "XYZ":
            ...
            WriteAll = False
            g_code.append(
                "(G-Code Ripper has mapped the X-Axis to the B-Axis      )")
        elif Wrap == "Y2B":
            AXIS = ["X", "B", "Z"]
            DECP = [PLACES_L, PLACES_R, PLACES_L]
            WriteAll = False
            g_code.append(
                "(G-Code Ripper has mapped the Y-Axis to the B-Axis      )")
        elif Wrap == "X2A":
            AXIS = ["A", "Y", "Z"]
            DECP = [PLACES_R, PLACES_L, PLACES_L]
            WriteAll = False
            g_code.append(
                "(G-Code Ripper has mapped the X-Axis to the A-Axis      )")

        if Wrap != "XYZ":
            g_code.append(
                "(A nominal stock radius of %f was used.             )" % (Rstock))
            g_code.append(
        ...
        for line in side:
            if line[0] == 1 or line[0] == 2 or line[0] == 3 or (line[0] == 0 and gen_rapids == False):
                ...
                if sqrt((dx+0j).real**2 + (dy+0j).real**2 + (dz+0j).real**2) > self.accuracy and gen_rapids == True:
                    ...
                    if no_variables == False:
                        ...
                        LINE = "G0"
                        LINE = self.app_gcode_line(
                            LINE, AXIS[2], z_safe, DECP[2], WriteAll)
                        if len(LINE) > 2:
                            g_code.append(LINE)
                    ...
                    LINE = "G0"
                    LINE = self.app_gcode_line(
                        LINE, AXIS[0], coordA[0], DECP[0], WriteAll)
                    LINE = self.app_gcode_line(
                        LINE, AXIS[1], coordA[1], DECP[1], WriteAll)
                    if len(LINE) > 2:
                        g_code.append(LINE)
                    ...
                    if float(coordA[2]) < float(z_safe):
                        if no_variables == False:
                            LINE = "G1"
                            LINE = self.app_gcode_line(
                                LINE, AXIS[2], coordA[2], DECP[2], WriteAll)
                            ...
                            LINE = self.app_gcode_line(
                                LINE, AXIS[2], coordA[2], DECP[2], WriteAll)
                            LINE = self.app_gcode_line(
                                LINE, "F", plunge_feed, PLACES_F, WriteAll)
                            if len(LINE) > 2:
                                g_code.append(LINE)
            ...
            if (line[0] == 1) or (line[0] == 2) or (line[0] == 3) or (line[0] == 0 and (gen_rapids == False)):
                try:
                    LAST0 = float(self.MODAL_VAL[AXIS[0]])
                except:
                    LAST0 = coordB[0]
                try:
                    LAST1 = float(self.MODAL_VAL[AXIS[1]])
                except:
                    LAST1 = coordB[1]
                try:
                    LAST2 = float(self.MODAL_VAL[AXIS[2]])
                except:
                    LAST2 = coordB[2]

                LINE = "G%d" % (line[0])
                LINE = self.app_gcode_line(
                    LINE, AXIS[0], coordB[0], DECP[0], WriteAll)
                LINE = self.app_gcode_line(
                    LINE, AXIS[1], coordB[1], DECP[1], WriteAll)
                LINE = self.app_gcode_line(
                    LINE, AXIS[2], coordB[2], DECP[2], WriteAll)
                if (line[0] == 1):
                    if ((LINE.find("A") > -1) or (LINE.find("B") > -1)) and (FSCALE == "Scale-Rotary") and (D012 > self.Zero):
                        ...
                        Feed_adj = line[3]
                    LINE = self.app_gcode_line(
                        LINE, "F", Feed_adj, PLACES_F, WriteAll)

                elif (line[0] == 2) or (line[0] == 3):
                    LINE = self.app_gcode_line(
                        LINE, "I", line[3][0], DECP[0], WriteAll)
                    LINE = self.app_gcode_line(
                        LINE, "J", line[3][1], DECP[1], WriteAll)
                    LINE = self.app_gcode_line(
                        LINE, "F", Feed_adj, PLACES_F, WriteAll)
                if len(LINE) > 2:
                    g_code.append(LINE)
    ...
    def get_arc_intersects(self, p1, p2, xsplit, cent, code):
        ...
        if abs(R-Rt) > self.accuracy:
            self.fmessage("Radius Warning: R1=%f R2=%f" % (R, Rt))
        ...
        xbeta, ybeta = self.Transform(
            p2[0]-cent[0], p2[1]-cent[1], radians(-theta))
        beta = self.Get_Angle2(xbeta, ybeta, code)

        if abs(beta) <= self.Zero:
            beta = 360.0

        xt, yt = self.Transform(
            xsplit-cent[0], ycross1-cent[1], radians(-theta))
        gt1 = self.Get_Angle2(xt, yt, code)

        xt, yt = self.Transform(
            xsplit-cent[0], ycross2-cent[1], radians(-theta))
    ...
    def arc2lines(self, p1, p2, cent, code, plane="17"):
        ...
        if code == 3:
            ...
            xbeta, ybeta = self.Transform(
                p2[xind]-cent[xind], p2[yind]-cent[yind], radians(-theta))
            X1 = p1[xind]
            Y1 = p1[yind]
            Z1 = p1[zind]
            zstart = Z1
        ...
        if code == 2:
            ...
            xbeta, ybeta = self.Transform(
                p1[xind]-cent[xind], p1[yind]-cent[yind], radians(-theta))
            X1 = p2[xind]
            Y1 = p2[yind]
            Z1 = p2[zind]
            zstart = Z1
    ...
    def EXPRESSION_EVAL(self, line):
        ...
        while s != -1:
            ...
            for iXOR in range(0, len(XOR_operation)):
                #####################################
                AND_operation = self.list_split(
                    XOR_operation[iXOR], "&")  # AND
                for iAND in range(0, len(AND_operation)):
                    #####################################
                    LE_operation = self.list_split(
                        AND_operation[iAND], "l")  # LE
                    for iLE in range(0, len(LE_operation)):
                        #####################################
                        LT_operation = self.list_split(
                            LE_operation[iLE], "<")  # LT
                        for iLT in range(0, len(LT_operation)):
                            #####################################
                            GE_operation = self.list_split(
                                LT_operation[iLT], "g")  # GE
                            for iGE in range(0, len(GE_operation)):
                                #####################################
                                GT_operation = self.list_split(
                                    GE_operation[iGE], ">")  # GT
                                for iGT in range(0, len(GT_operation)):
                                    #####################################
                                    NE_operation = self.list_split(
                                        GT_operation[iGT], "!")  # NE
                                    for iNE in range(0, len(NE_operation)):
                                        #####################################
                                        EQ_operation = self.list_split(
                                            NE_operation[iNE], "=")  # EQ
                                                ...
                                                for cnt in range(1, len(subtract)):
                                                    if subtract[cnt-1] == []:
                                                        ...
                                                        subtract[cnt][0] = - \
                                                            float(
                                                                subtract[cnt][0])
                                                for i in range(subtract.count('')):
                                                    subtract.remove('')
                                                for isub in range(0, len(subtract)):
                                                    #####################################
                                                    multiply = self.list_split(
                                                        subtract[isub], "*")
                                                    for imult in range(0, len(multiply)):
                                                        #####################################
                                                        divide = self.list_split(
                                                            multiply[imult], "/")
                                                        for idiv in range(0, len(divide)):
                                                            ...
                                                            for imod in range(0, len(mod)):
                                                                #####################################
                                                                power = self.list_split(
                                                                    mod[imod], "^")
                                                                for ipow in range(0, len(power)):
                                                                    if power[ipow] == []:
                                                                        MSG = "ERROR EXP-3: Unable to evaluate expression: %s" % (
                                                                            line_in)
                                                                        raise ValueError(
                                                                            MSG)
                                                            ...
                                                            for k in range(len(mod), 1, -1):
                                                                res_mod = mod[k -
                                                                              2] % res_mod
                                                                self.fmessage(
                                                                    "     MOD"+str(mod)+"="+str(res_mod))
                                                            divide[idiv] = res_mod
                                                        #####################################
                                                        res_divide = divide[0]
                                                        for k in range(1, len(divide), 1):
                                                            res_divide = res_divide / \
                                                                divide[k]
                                                            if P == True:
                                                                self.fmessage(
                                                                    "  DIVIDE"+str(divide)+"="+str(res_divide))
                                                    ...
                                                    for k in range(1, len(multiply)):
                                                        ...
                                                        if P == True:
                                                            self.fmessage(
                                                                "MULTIPLY"+str(multiply)+"="+str(res_multiply))
                                                    subtract[isub] = res_multiply
                                                #####################################
                                                res_subtract = subtract[0]
                                                for k in range(1, len(subtract)):
                                                    res_subtract = res_subtract - \
                                                        subtract[k]
                                                    if P == True:
                                                        self.fmessage(
                                                            "SUBTRACT"+str(subtract)+"="+str(res_subtract))
                                        ...
                                        for k in range(1, len(EQ_operation), 1):
                                            ...
                                            if P == True:
                                                self.fmessage(
                                                    "      EQ"+str(EQ_operation)+"="+str(res_EQ))
                                    ...
                                    for k in range(1, len(NE_operation), 1):
                                        ...
                                        if P == True:
                                            self.fmessage(
                                                "      NE"+str(NE_operation)+"="+str(res_NE))
                                ...
                                for k in range(1, len(GT_operation), 1):
                                    ...
                                    if P == True:
                                        self.fmessage(
                                            "      GT"+str(GT_operation), "="+str(res_GT))
                            ...
                            for k in range(1, len(GE_operation), 1):
                                ...
                                if P == True:
                                    self.fmessage(
                                        "      GE"+str(GE_operation)+"="+str(res_GE))
                        ...
                        for k in range(1, len(LT_operation), 1):
                            ...
                            if P == True:
                                self.fmessage(
                                    "      LT"+str(LT_operation)+"="+str(res_LT))
                    ...
                    for k in range(1, len(LE_operation), 1):
                        ...
                        if P == True:
                            self.fmessage(
                                "      LE"+str(LE_operation)+"="+str(res_LE))
                ...
                for k in range(1, len(AND_operation), 1):
                    ...
                    if P == True:
                        self.fmessage(
                            "      AND"+str(AND_operation)+"="+str(res_AND))
            ...
            for k in range(1, len(XOR_operation), 1):
                ...
                if P == True:
                    self.fmessage(
                        "      XOR"+str(XOR_operation)+"="+str(res_XOR))
...
if __name__ == "__main__":
    g_rip = G_Code_Rip()
    filename = "Z:\\text.ngc"
    MSG = g_rip.Read_G_Code(filename, XYarc2line=True,
                            arc_angle=2, units="in", Accuracy="")
    print(MSG)
