...
import re
# local library
import k40_web.laser_controller.inkex as inkex
import k40_web.laser_controller.simplestyle as simplestyle
import k40_web.laser_controller.simpletransform as simpletransform
import k40_web.laser_controller.cubicsuperpath as cubicsuperpath
import k40_web.laser_controller.cspsubdiv as cspsubdiv
...
def run_external(cmd, timeout_sec):
    ...
    try:
        ...
        raise Exception("\n%s\n\nExecutable Path:\n%s" % (e, cmd[0]))
    if timeout_sec > 0:
        def kill_proc(p): return kill_sub_process(p, timeout_sec, FLAG)
...
class SVG_READER(inkex.Effect):
    ...
    def process_shape(self, node, mat, group_stroke=None):
        ...
        if class_val:
            ...
            for cv in class_val.split(' '):
                if css_data != "":
                    css_data = self.CSS_values.get_css_value(
                        tag_type, cv)+";"+css_data
                else:
        ...
        if stroke_outside:
            ...
            if simplestyle.isColor(col):
                ...
                (new_val, changed, k40_action) = self.colmod(
                    c[0], c[1], c[2], path_id)
            else:
                new_val = col
            if changed:
        ...
        if changed:
            ...
            if node.tag == inkex.addNS('path', 'svg'):
                ...
                if max(rx, ry) > 0.0:
                    ...
                    L2 = "M %f,%f %f,%f " % (
                        x+width, y+ry, x+width, y+height-ry)
                    C2 = "A %f,%f 0 0 1 %f,%f" % (rx, ry, x+width-rx, y+height)
                    L3 = "M %f,%f %f,%f " % (
                        x+width-rx, y+height, x+rx, y+height)
                ...
                d = "M %f,%f A   %f,%f 0 0 1 %f,%f   %f,%f 0 0 1 %f,%f   %f,%f 0 0 1 %f,%f   %f,%f 0 0 1 %f,%f Z" % (
                    cx+r, cy, r, r, cx, cy+r,  r, r, cx-r, cy,  r, r, cx, cy-r, r, r, cx+r, cy)
                p = cubicsuperpath.parsePath(d)
                ...
                d = "M %f,%f A   %f,%f 0 0 1 %f,%f   %f,%f 0 0 1 %f,%f   %f,%f 0 0 1 %f,%f   %f,%f 0 0 1 %f,%f Z" % (
                    cx+rx, cy, rx, ry, cx, cy+ry,  rx, ry, cx-rx, cy,  rx, ry, cx, cy-ry, rx, ry, cx+rx, cy)
                p = cubicsuperpath.parsePath(d)
    ...
    def process_group(self, group):
        ...
        if trans:
            self.groupmat.append(simpletransform.composeTransform(
        ...
        for node in group:
            if node.tag == inkex.addNS('g', 'svg') or node.tag == inkex.addNS('switch', 'svg'):
                ...
                self.process_shape(
                    node, self.groupmat[-1], group_stroke=stroke_group)
        if trans:
            self.groupmat.pop()
    ...
    def Make_PNG(self):
        ...
        if self.inkscape_exe != None:
            try:
                ...
                error_text = "%s" % (e)
                raise Exception(
                    "Inkscape Execution Failed (while making raster data).\n%s" % (error_text))
        else:
    ...
    def convert_text2paths(self):
        ...
        if self.inkscape_exe != None:
            try:
                ...
                raise Exception(
                    "Inkscape Execution Failed (while converting text to paths).\n\n"+str(e))
        else:
            raise Exception("Inkscape Not found.")
        try:
            shutil.rmtree(tmp_dir)
    ...
    def set_size(self, pxpi, viewbox, scale):
        ...
        self.document.getroot().set('viewBox', '%f %f %f %f' %
    ...
    def make_paths(self, txt2paths=False):
        ...
        ## GET VIEWBOX ##
        #################
        view_box_array = self.document.getroot().xpath(
            '@viewBox', namespaces=inkex.NSS)  # [0]
        if view_box_array == []:
            view_box_str = None
        else:
