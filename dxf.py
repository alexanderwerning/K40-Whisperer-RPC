...
class DXF_CLASS:
    def __init__(self):
        ...
        # String (255 characters maximum; less for Unicode strings)
...
# fd.close()
    ...
    def add_coords(self, line, offset, scale, rotate, color=None, layer=None):
        ...
        try:
            ...
            try:
                for lay in layer:
                    Layer_Engrave = (
                        str.find(lay.upper(), 'ENGRAVE') != -1) or Layer_Engrave
            except:
                pass
    ...
    def eval_entity(self, e, bl, lin_tol=.001, offset=[0, 0], scale=[1, 1], rotate=0):
        ...
        if e.type == "LINE":
            ...
            x1 = e.data["11"]
            y1 = e.data["21"]
            self.add_coords([x0, y0, x1, y1], offset,
                            scale, rotate, color, layer)
        ############# ARC #############
        elif e.type == "ARC":
            x = e.data["10"]
            y = e.data["20"]
        ...
        ######### LWPOLYLINE ##########
                ...
                if flag == 0:
                    ...
                    if bulge0 != 0:
                        bcoords = self.bulge_coords(
                            x0, y0, x1, y1, bulge0, lin_tol=lin_tol)
                        for line in bcoords:
                            self.add_coords(line, offset, scale,
                                            rotate, color, layer)
                    else:
                        self.add_coords([x0, y0, x1, y1], offset,
                                        scale, rotate, color, layer)
            ...
            if (e.data["70"] & 1):  # check if bit-code 1 is set for closed polygon
                ...
                if bulge0 != 0:
                    bcoords = self.bulge_coords(
                        x0, y0, x1, y1, bulge1, lin_tol=lin_tol)
                    for line in bcoords:
                        self.add_coords(line, offset, scale,
                                        rotate, color, layer)
                else:
        ...
        ########### ELLIPSE ###########
            ...
            while phi < end_r:
                ...
                if delta > lin_tol:
                    ...
                    phi += step
                    self.add_coords([x1, y1, x2, y2], offset,
                                    scale, rotate, color, layer)
                    step = step*2
                    x1 = x2
                    y1 = y2
        ...
        ########### SEQEND ###########
            ...
            if (self.POLY_FLAG == 1):  # None):
                ...
                if (self.POLY_CLOSED == 1):
                    ...
                    if self.bulge != 0:
                        bcoords = self.bulge_coords(
                            x0, y0, x1, y1, self.bulge, lin_tol=lin_tol)
                        for line in bcoords:
                            self.add_coords(line, offset, scale,
                                            rotate, color, layer)
                    else:
        ...
        ########### INSERT ###########
            ...
            for e in bl.blocks[key].entities:
                self.eval_entity(e, bl, lin_tol, offset=[xoff, yoff], scale=[
                                 xscale, yscale], rotate=rotate)
        ...
        ########### END INSERT ###########
            ...
            self.add_coords([x0, y0, x1, y1], offset,
                            scale, rotate, color, layer)
            self.add_coords([x1, y1, x3, y3], offset,
                            scale, rotate, color, layer)
            self.add_coords([x3, y3, x2, y2], offset,
                            scale, rotate, color, layer)
            self.add_coords([x2, y2, x0, y0], offset,
                            scale, rotate, color, layer)
    ...
    def WriteDXF(self, close_loops=False):
        ...
        dxf_code.append("999")
        dxf_code.append(
            "DXF created by dxf library <by Scorch, www.scorchworks.com>")
